...
function getLiRuncount(){
	var client=createMySQLClient();
	client.query("select value from ssc_params where name='LiRuncount'", function(err, data){
		if(err){
			LiRuncount=0;
		}else{
			data.forEach(function(v){
...
function getplaycount(data, conf){
	var client=createMySQLClient();
	client.query("select count(username) from ssc_bets where actionNo=?", [data.number], function(err, data){
		if(err){
			playcount=0;
		}else{
			data.forEach(function(v){
...
function liRunData(data, conf){
	...
	getLiRunLv();
	var client=createMySQLClient();
	client.query("select actionNum,playedId,actionData,weiShu,mode,beiShu,bonusProp from ssc_bets where isDelete=0 and lotteryNo='' and type=? and actionNo=?", [data.type, data.number], function(err, bets){
		if(err){
			log("读取投注出错："+err);
		}else{
...
function calcJ(data, flag){
	var client=createMySQLClient();
	sql="select id,playedId,actionData,weiShu from ssc_bets where isDelete=0 and type=? and actionNo=?";
